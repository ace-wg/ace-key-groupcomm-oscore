...
* In the Token Transfer Response, the 'kdcchallenge' parameter contains a dedicated nonce N_S generated by the Group Manager. As to the N\_S value, it is RECOMMENDED to be at least 8-byte long and it is RECOMMENDED to be a random value. The joining node can use this nonce in order to prove the possession of its own private key upon joining the group (see {{ssec-join-req-sending}} of this document).
...
* The 'cnonce' parameter contains a dedicated nonce N_C generated by the joining node. As to the N\_C value, it is RECOMMENDED to be at least 8-byte long and it is RECOMMENDED to be a random value.
...
* The 'cnonce' parameter contains a new dedicated nonce N\_C generated by the joining node. As to the N\_C value, it is RECOMMENDED to be at least 8-byte long and it is RECOMMENDED to be a random value.
...
* The 'kdc_nonce' parameter MUST be present, specifying the dedicated nonce N_KDC generated by the Group Manager. As to the N_KDC value, it is RECOMMENDED to be at least 8-byte long and it is RECOMMENDED to be a random value.
...
* 'cnonce': encoded as a CBOR byte string, whose value is a dedicated nonce N_C generated by the Client. As to the N_C value, it is RECOMMENDED to be at least 8-byte long and it is RECOMMENDED to be a random value.
...
This is practically ensured if parties generate nonces as recommended in this document. If a party generates nonces that are not random values (e.g., by using a counter), the party ought to ensure that values are not reused, even in case of reboot and loss of state.
