...
* In the Token Transfer Response, the 'kdcchallenge' parameter contains a fresh challenge N_S newly generated by the Group Manager. As to the N\_S value, it is RECOMMENDED to be at least 8-byte long and it is RECOMMENDED to be a random value. The joining node can use this challenge in order to prove the possession of its own private key upon joining the group (see {{ssec-join-req-sending}} of this document).
...
* The 'cnonce' parameter contains a fresh challenge N_C newly generated by the joining node. As to the N\_C value, it is RECOMMENDED to be at least 8-byte long and it is RECOMMENDED to be a random value.
...
* The 'kdc_nonce' parameter MUST be present, specifying the nonce N_KDC generated by the Group Manager. As to the N_KDC value, it is RECOMMENDED to be at least 8-byte long and it is RECOMMENDED to be a random value.
...
* 'cnonce': encoded as a CBOR byte string, whose value is a fresh challenge N_C newly generated by the Client signature verifier. As to the N_C value, it is RECOMMENDED to be at least 8-byte long and it is RECOMMENDED to be a random value.
...
As to the N\_C value, it is RECOMMENDED to be at least 8-byte long and it is RECOMMENDED to be a random value. Furthermore, N\_C is always conveyed in the 'cnonce' parameter of the Join Request, which is always sent over the secure communication association between the joining node and the Group Manager.
